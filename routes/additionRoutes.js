...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(400)
        .render("addition", { error: "Failed to add produce item" });
    }
  }
);
...
router.get(
  ...
  async (req, res) => {
    try {
      const items = await Produce.find({ branch: req.user.branch }).sort({
        createdAt: -1,
      }); // Proper sorting
      ...
      res
        .status(400)
        .render("error", { message: "Failed to load produce list" });
    }
  }
);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!produce) {
        return res
          .status(404)
          .render("error", { message: "Produce not found" });
      }
      ...
      res
        .status(400)
        .render("error", { message: "Failed to load update page" });
    }
  }
);
